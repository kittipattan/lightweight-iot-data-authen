...
class Leader(IoT):
    ...
    def verifyMutAuthProof(self, pt: Tuple[bytes, datetime]):
        ...
        try:
            (pairing_n, timestamp_prime) = AESCipher(self.p_1).decrypt(proof).split('||||')
    ...
    def resMutAuth(self):
        ...
        proof = AESCipher(pairing_p_2).encrypt(f"{self.nonces['n'].hex()}||||{self.pairing_timestamp}")

        return proof
    
    def exchangeKey(self):
        self.localDatabase[self.pairing_id] = sha256(self.nonces['pairing_n'] + self.nonces['n']).digest()
    
    # Our scheme
    def sendSecToIoT(self, iot_id: int, secret: bytes, partial_ciphered_key: bytes):
        enc_secret = AESCipher(self.localDatabase[iot_id]).encrypt(f"{secret.hex()}")
        return (enc_secret, partial_ciphered_key)
    
    
class GroupLeader(Leader, GroupIoT):
    def __init__(self, id, gid, seed, data, n_challenge=64):
        super().__init__(id, gid, seed, data, n_challenge)
        self.encSecret = None
    ...
    def __encryptSec(self):
        self.encSecret = AESCipher(self.gk).encrypt(f"{self.secret.hex()}")
        
    def recvSecFromFog(self, enc_packet):
        (secret, ciphered_keys) = super().recvSecFromFog(enc_packet)
        self.__encryptSec()
        return (secret, ciphered_keys)
